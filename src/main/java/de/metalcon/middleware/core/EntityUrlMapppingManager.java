...
public class EntityUrlMapppingManager {
	...
	private final Map<Muid, Set<String>> muidToMapping;

	private final Map<String, Muid> mappingToMuidBand;

	private final Map<String, Muid> mappingToMuidCity;

	private final Map<String, Muid> mappingToMuidEvent;

	private final Map<String, Muid> mappingToMuidGenre;

	private final Map<String, Muid> mappingToMuidInstrument;

	private final Map<Muid, Map<String, Muid>> mappingToMuidRecord;

	private final Map<String, Muid> mappingToMuidTour;

	private final Map<Muid, Map<String, Muid>> mappingToMuidTrack;

	private final Map<String, Muid> mappingToMuidUser;

	private final Map<String, Muid> mappingToMuidVenue;
	...
	public void registerMuid(Muid muid) {
		...
		// @formatter:off
		case BAND:
			registerMuidBand(muid, (Band) entity);
			break;
		case CITY:
			registerMuidCity(muid, (City) entity);
			break;
		case EVENT:
			registerMuidEvent(muid, (Event) entity);
			break;
		case GENRE:
			registerMuidGenre(muid, (Genre) entity);
			break;
		case INSTRUMENT:
			registerMuidInstrument(muid, (Instrument) entity);
			break;
		case RECORD:
			registerMuidRecord(muid, (Record) entity);
			break;
		case TOUR:
			registerMuidTour(muid, (Tour) entity);
			break;
		case TRACK:
			registerMuidTrack(muid, (Track) entity);
			break;
		case USER:
			registerMuidUser(muid, (User) entity);
			break;
		case VENUE:
			registerMuidVenue(muid, (Venue) entity);
			break;
		// @formatter:on
	...
	public Muid getMuid(EntityType entityType, Map<String, String> pathVars)
		...
		case CITY:
			return getMuidCity(pathVars);
		case EVENT:
			return getMuidEvent(pathVars);
		case GENRE:
			return getMuidGenre(pathVars);
		case INSTRUMENT:
			return getMuidInstrument(pathVars);
		case RECORD:
			return getMuidRecord(pathVars);
		case TOUR:
			return getMuidTour(pathVars);
		case TRACK:
			return getMuidTrack(pathVars);
		case USER:
			return getMuidUser(pathVars);
		case VENUE:
			return getMuidVenue(pathVars);
			// @formatter:on
	...
	private void registerMuidRecord(Muid muid, Record record) {
		...
		if (band != null) {
			...
			mappingToMuid = new HashMap<String, Muid>();
			mappingToMuidRecord.put(Muid.EMPTY_BAND_MUID, mappingToMuid);
		}
	...
	private void registerMuidTrack(Muid muid, Track track) {
		...
		if (record != null) {
			...
			mappingToMuid = new HashMap<String, Muid>();
			mappingToMuidTrack.put(Muid.EMPTY_BAND_MUID, mappingToMuid);
		}
	...
	private Muid getMuidTrack(Map<String, String> pathVars)
		...
		Map<String, Muid> record = mappingToMuidTrack
				.get(getMuidRecord(pathVars));
		if (record == null) {
			return null;
		}
